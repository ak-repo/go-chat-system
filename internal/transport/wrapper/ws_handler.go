...
import (
	...
	ws "github.com/ak-repo/go-chat-system/internal/transport/websocket"
...
func isWSOriginAllowed(origin string) bool {
	...
	if len(allowedOrigins) == 0 {
		// Construct default origin: http://localhost:3000
		allowedOrigins = []string{fmt.Sprintf("http://%s:%d", config.Config.CORS.Host, config.Config.CORS.Port)}
	}
